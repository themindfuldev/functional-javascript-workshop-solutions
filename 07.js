...
function recursiveReduce(arr, fn, prev, index) {
    if (index > arr.length-1) {
        return prev;
    }
    const step = fn(prev, arr[index], index, arr);
    return recursiveReduce(arr, fn, step, ++index);
}

function reduce(arr, fn, initial) {
    return recursiveReduce(arr, fn, initial, 0);
}
